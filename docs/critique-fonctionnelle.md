...
### F9 ✅ — Il n'y avait aucune sauvegarde (clos 2026-09-14)
...
**Mise à jour 2026-09-13 :** pgBackRest était en fait déjà en place sur `pg_main` (chiffrement AES-256, WAL archiving continu) mais jamais planifié avec succès — la crontab existante appelait `docker exec` sans `--user postgres` et échouait à **chaque exécution, silencieusement, depuis le début** (un seul backup complet du 22 août a jamais réussi, à la main). Élargi, corrigé et déployé dans [`mycomicsmanager-config`](https://github.com/slucky31/mycomicsmanager-config) (config committée, l'instance n'appartient pas à LoreAI — ADR 0009) : repo1 déplacé sur disque USB (pas la carte SD, qui aurait partagé le même point de panne que les données live), `repo1-bundle=y` (2355 → 10 fichiers par backup), miroir chiffré vers Google Drive. Couvre les deux points ci-dessus côté base — **toutes** les bases de l'instance (`loreai`, `mycomicsmanager_prod`, `miniflux`), pas seulement `loreai`, puisque c'est un backup physique du cluster entier. **Restauration réelle testée avec succès le 2026-09-13** (E1, `reste-a-tester.md`) : les 4 vraies bases restaurées et vérifiées.

**Mise à jour 2026-09-14 :** le second actif (le `.env` de `mcm8` et les autres secrets locaux — mots de passe PostgreSQL, passphrase pgBackRest, jeton rclone Google Drive) est traité de la même façon : `gpg --symmetric` (AES-256, passphrase distincte de celle de pgBackRest) puis envoi vers Google Drive (`secrets-backup/` dans `mycomicsmanager-config`, cron hebdomadaire). Restauration testée : les 8 fichiers attendus déchiffrés et vérifiés. **Les deux points de F9 sont clos.**
...
|---|---|---|---|---|
| **O7** | ~~**Sauvegarde réelle**~~ **Fait** — pgBackRest (repo1 sur disque USB, miroir Google Drive) et `.env`/secrets (`gpg` + Google Drive) déployés et testés sur `mcm8` (2026-09-13/14) | 3 | 1 | F9 — clos |
