...
| | |
|---|---|
| **Lot en cours** | **Lot 4 ([#45](https://github.com/slucky31/LoreAI/issues/45)) mergé sur `main`** (PR [#66](https://github.com/slucky31/LoreAI/pull/66) → release v0.13.0) **et signalé déployé sur `mcm8`, vérification en production pas encore faite.** `IContentFetcher`/`HttpContentFetcher` (AngleSharp + heuristique maison, best-effort strict), colonnes `ContentText`/`ContentFetchedAtUtc`/`ContentStatus`/`WordCount`/`Summary` sur `Articles` (migration `AddContentFetchAndSummary`), champ `summary` ajouté au tool `classify` (S2, un seul appel LLM), `max_tokens` 300→800, `ReadingTimeEstimator` (L2), interrupteur `Worker__FetchArticleContent`, `Classifier__SummaryModel` posé comme point d'extension non consommé (D6). Log version+environnement au démarrage (#65, Worker+MCP). 238/238 tests passent. |
| **Prochain geste** | Vérifier le lot 4 en production sur `mcm8` : confirmer la version au démarrage (#65), confirmer que le prochain cycle réel peuple bien `ContentText`/`ContentStatus`/`WordCount`/`Summary` sur un article, puis trancher **#34 (cache de prompt)** — posé mais pas mesuré, pas de clé Anthropic réelle disponible pendant le développement. Requête corrigée (la colonne est `ClassificationRawResponse`, pas `classification_raw_response` — Npgsql/EF Core garde la casse C#, les identifiants sont sensibles à la casse dès qu'ils sont quotés) :<br>`SELECT "Id", "ClassificationRawResponse"->'usage' AS usage FROM "Articles" WHERE "ClassifiedAtUtc" IS NOT NULL ORDER BY "ClassifiedAtUtc" DESC LIMIT 5;`<br>Si `cache_creation_input_tokens`/`cache_read_input_tokens` restent à 0 → seuil non atteint, sujet clos jusqu'au backfill. Indépendamment : le worker sur `mcm8` restait en v0.8.0 avant ce déploiement — à confirmer qu'il tourne bien désormais en 0.13.0, et que le lot 2 (compte-rendu Discord, retrait email, `WeeklyInsightsJob`) est enfin observable en production du même coup. |
...
| **Bloqué par** | Rien. Le Pi (`mcm8`) est en ligne ; déploiement du lot 4 signalé fait, non encore vérifié — voir « Prochain geste ». |
