...
| | |
|---|---|
| **Lot en cours** | **Lot 5 ([#46](https://github.com/slucky31/LoreAI/issues/46)) mergé, déployé et partiellement vérifié en production sur `mcm8`** (PR [#69](https://github.com/slucky31/LoreAI/pull/69) → release v0.14.0). Vérifié le 2026-08-24 via le MCP en conditions réelles (1333 items indexés) : `search_items` (Q2, plein texte réel, résultats pertinents), `find_similar` (S5, exclut bien la source), `export_item` (S8, frontmatter correct, gère bien le cas « jamais classifié »), `catalog_tools`/`list_tools` répondent sans erreur. **Encore à vérifier** : `catalog_tools`/`tool_card` (S7) — le catalogue est vide, normal tant qu'aucun cycle post-déploiement n'a classé un article `ATester` avec un nom d'outil ; à recontrôler après quelques cycles de 15 min. `MonthlyReviewJob` (S4) — cron 1er du mois 5h UTC, pas de `RunOnceAtStart()` : premier passage réel le **2026-09-01**, chercher `loreai-revue-mensuelle-2026-08.md` sur Discord. |
| **Prochain geste** | Repasser vérifier `catalog_tools` dans quelques cycles (dès qu'un article `ATester` avec nom d'outil est traité), et `MonthlyReviewJob` le 2026-09-01. Indépendamment : lot 2 ([#43](https://github.com/slucky31/LoreAI/issues/43)) vérifié aux 2/3 — seul `WeeklyInsightsJob` (N1/N2/N5/S3/S6) reste à vérifier en prod, cron dimanche 3h/4h UTC : **on attend le prochain passage naturel, dimanche 2026-08-30**. Chercher dans Discord un fichier attaché `loreai-insights-2026-08-30.md`. |
