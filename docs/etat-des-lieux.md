...
| **Prochain geste** | Lot 4 clos. Reste, indépendamment : confirmer que le lot 2 (compte-rendu Discord, retrait email, `WeeklyInsightsJob`) est bien observable en production — non revérifié explicitement pendant cette session, seul le pipeline de classification l'a été. Sinon, prochain morceau de la roadmap : lot 5 ([#46](https://github.com/slucky31/LoreAI/issues/46), synthèse et projections) ou lots 7/8 (RSS/Gmail, phase 2). |
