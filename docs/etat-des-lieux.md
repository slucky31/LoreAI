...
| **Prochain geste** | Lot 3 ([#44](https://github.com/slucky31/LoreAI/issues/44)) : code + CI/CD complets. `src/LoreAI.Mcp` posé, testé (204/204) et vérifié à la main (serveur lancé en local, image Docker construite et testée au curl : 401 sans jeton, `initialize`/`tools/list`/`tools/call` OK) — 5 outils sur 7, `find_similar`/`reading_queue` explicitement non implémentés (Q2/pgvector, scoring lot 6). `Dockerfile`, entrée `docker-compose.yml`, `.env.example` et section 11 de [`docs/deploiement-raspberry-pi.md`](deploiement-raspberry-pi.md) écrits. `ci.yml`/`cd.yml` construisent et publient désormais `ghcr.io/slucky31/loreai-mcp` **avec la même version que `loreai-worker`** (un seul `<Version>` dans `Directory.Build.props`, ADR 0008 — les deux images sortent toujours ensemble). Rôle `loreai_ro` déjà provisionné depuis le lot 0, rien à refaire côté base. **Reste à faire :** merger sur `main` pour déclencher une release (v0.12.0) et vérifier que l'image `loreai-mcp` sort bien, puis activer sur `mcm8` (section 11). Déploiement v0.11.0 sur `mcm8` et vérification en production (compte-rendu Discord, absence du digest email, premier rapport hebdomadaire) toujours en attente, indépendamment du lot 3. |
