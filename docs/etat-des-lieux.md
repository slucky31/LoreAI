...
**Dernière mise à jour :** 2026-08-29 · **Version publiée :** 0.18.1 (#90 mergée, release en attente)
...
| | |
|---|---|
| **Lot en cours** | **Lot 7 livré et déployé sur `mcm8`** ([#48](https://github.com/slucky31/LoreAI/issues/48)+[#78](https://github.com/slucky31/LoreAI/issues/78), PR [#87](https://github.com/slucky31/LoreAI/pull/87)/[#88](https://github.com/slucky31/LoreAI/pull/88)/[#89](https://github.com/slucky31/LoreAI/pull/89)) : Miniflux tourne réellement sur `mcm8` (port hôte `5100`, base dédiée provisionnée), `curl .../v1/entries` confirmé fonctionnel. **En parallèle, L5 implémentée cette session** (validation explicite donnée par l'utilisateur pour la première écriture hors « Non trié ») : `ReadingQueueTaggingJob` pose/retire un tag Raindrop (`cette-semaine`, configurable) sur la file de lecture hebdomadaire — jamais de déplacement de collection ni de réécriture de note, `RaindropSnapshot` relit l'état réel à chaque passage. Désactivé par défaut (`Worker__ReadingQueueTaggingEnabled=false`). **Bug de réconciliation découvert et corrigé en cours de route** (PR [#90](https://github.com/slucky31/LoreAI/pull/90)) : `ReconciliationJob` interrogeait Raindrop avec `Articles.Id` (id technique, généré par la base depuis le lot 8) au lieu de `SourceId` (l'id Raindrop réel) — faussait potentiellement N3/N4/L1/L4 depuis le lot 8. 391/391 tests verts sur `main`. |
| **Prochain geste** | Sur `mcm8` : (1) terminer l'activation Miniflux — ajouter les flux RSS souhaités via l'UI, seeder `PollingStates SourceType='Feed'` (`'0'` si aucune entrée encore collectée), `Worker__FeedIngestionEnabled=true`. (2) Ouvrir et merger la PR L5 (pas encore poussée au moment de cette mise à jour). (3) **Activer le connecteur Gmail** (lot 8, toujours désactivé en prod faute de projet Google Cloud/client OAuth créé). Une fois Miniflux ou Gmail actif en prod : **Lot 9 (veille automatique, [#50](https://github.com/slucky31/LoreAI/issues/50)) est débloqué**. |
| **Dernière décision** | **L5 validée explicitement en session (2026-08-29) : mécanisme = tag Raindrop, pas une vraie collection.** Un raindrop n'appartient qu'à une seule collection à la fois — y déplacer les articles de la file de lecture les retirerait de leur collection thématique déjà assignée par la classification. Le tag laisse l'article en place, juste marqué en plus (recherche sauvegardée Raindrop). |
| **Bloqué par** | Rien. Le Pi (`mcm8`) est en ligne, Miniflux y tourne. |

## Ce qui tourne aujourd'hui

**Sur `main` :** le worker classe la collection « Non trié » toutes les 15 min (Claude Haiku, tool-use forcé), applique tags et déplacements dans Raindrop, alerte sur Discord les articles `ATester` / `Haute`, et envoie un compte-rendu Discord à la fin de chaque cycle ayant traité au moins un article. Plus d'email : le canal a été retiré (D3, ADR 0013). Persistance PostgreSQL (instance mutualisée sur `mcm8`, EF Core), modèle `Item` générique multi-sources, journal `CycleRuns` et healthcheck Docker (lot 0). `LibraryIndexingJob` (hebdomadaire) indexe en lecture seule toute la bibliothèque Raindrop déjà triée par l'utilisateur (lot 1) ; `WeeklyInsightsJob` (hebdomadaire, juste après) lit cet index et envoie un digest Discord natif (embeds, doublons, tags à nettoyer, collections déséquilibrées, tendances, coût LLM) — zéro appel LLM, zéro écriture (lot 2, revu au lot 7/O6). Connecteurs Gmail (lot 8) et RSS/Miniflux (lot 7) écrits et testés ; Miniflux est déployé sur `mcm8` mais l'ingestion (`Worker__FeedIngestionEnabled`) et Gmail restent à activer (voir « Prochain geste »). `ReadingQueueTaggingJob` (L5, lot 8) écrit et testé, désactivé par défaut. Déploiement par image `ghcr.io` multi-arch, le Pi ne fait que `pull`.

**Réellement en production sur `mcm8` :** rapproché de `main` pendant cette session (déploiement Miniflux en cours), mais l'état exact des flags d'activation (`Worker__FeedIngestionEnabled`, `Worker__EmailIngestionEnabled`, `Worker__ReadingQueueTaggingEnabled`) n'a pas été revérifié en fin de session — se fier au `.env` réel sur `mcm8`, pas à cette ligne.
