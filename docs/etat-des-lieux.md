...
**Dernière mise à jour :** 2026-09-14 · **Version publiée :** 0.20.0
...
| **Prochain geste** | **Sauvegarde ([#37](https://github.com/slucky31/LoreAI/issues/37)) close** : base (pgBackRest, repo1 sur disque USB, full quotidien, miroir Google Drive, E1 testée) **et** `.env`/secrets (`gpg` + Google Drive, `secrets-backup/` dans `mycomicsmanager-config`, restauration testée) déployés sur `mcm8` le 2026-09-13/14. **Migration PostgreSQL 16 → 18.6 réalisée le 2026-09-14** ([#101](https://github.com/slucky31/LoreAI/issues/101), ADR 0009) — dump/restore, données et sauvegarde revérifiées sur la nouvelle version. Lot 11 continue avec : journal d'identité au démarrage étendu aux jobs planifiés ([#65](https://github.com/slucky31/LoreAI/issues/65)), healthcheck MCP ([#68](https://github.com/slucky31/LoreAI/issues/68)), Renovate débloqué ([#40](https://github.com/slucky31/LoreAI/issues/40)), logs `--add-watch-topic` ([#97](https://github.com/slucky31/LoreAI/issues/97)). En même temps : fermer les 5 issues livrées (#31, #35, #71, #73, #75) et les 3 périmées (#33, #36, #39). |
...
1. ~~**Il n'y a aucune sauvegarde.**~~ **Clos** (2026-09-13/14) : base (pgBackRest, repo1 chiffré sur disque USB, full quotidien, miroir Google Drive, restauration réelle E1 sur les 4 vraies bases) **et** `.env`/secrets de `mcm8` (7 secrets dont un refresh token OAuth Google, chiffrés `gpg` + Google Drive, restauration testée) — les deux volets de F9 sont fermés et vérifiés.
