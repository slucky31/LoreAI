...
1. **La sauvegarde de la base est déployée et testée** (2026-09-13) : pgBackRest, repo1 chiffré sur disque USB (pas la carte SD), full quotidien planifié, miroir Google Drive, restauration réelle (E1) réussie sur les 4 vraies bases. Il reste la copie chiffrée du `.env` de `mcm8` (7 secrets qui n'existent nulle part ailleurs, dont un refresh token OAuth Google) — non commencée, et toujours le risque le plus élevé du projet tant que ce point n'est pas clos.
