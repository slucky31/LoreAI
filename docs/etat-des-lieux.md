...
| | |
|---|---|
| **Lot en cours** | **Lot 4 ([#45](https://github.com/slucky31/LoreAI/issues/45)) codé sur la branche `feat/lot4-content-summary`, pas encore poussé ni en PR.** `IContentFetcher`/`HttpContentFetcher` (AngleSharp + heuristique maison, best-effort strict), colonnes `ContentText`/`ContentFetchedAtUtc`/`ContentStatus`/`WordCount`/`Summary` sur `Articles` (migration `AddContentFetchAndSummary`), champ `summary` ajouté au tool `classify` (S2, un seul appel LLM), `max_tokens` 300→800, `ReadingTimeEstimator` (L2), interrupteur `Worker__FetchArticleContent`, `Classifier__SummaryModel` posé comme point d'extension non consommé (D6). Log version+environnement au démarrage (#65, Worker+MCP). 238/238 tests passent. |
| **Prochain geste** | Pousser la branche, ouvrir la PR (titre `feat:` conventionnel), merger → release automatique. **Décision #34 (cache de prompt) posée mais pas tranchée** : un marqueur `cache_control` a été ajouté sur le tool `classify` (aucune restructuration du prompt), mais la mesure réelle (`cache_creation_input_tokens`/`cache_read_input_tokens`) n'a pas pu être faite dans cette session — pas de clé Anthropic réelle disponible. Une fois déployé sur `mcm8`, lancer un cycle réel puis : `SELECT classification_raw_response->'usage' FROM "Articles" WHERE classified_at_utc IS NOT NULL ORDER BY classified_at_utc DESC LIMIT 5;`. Si les deux compteurs restent à 0 → seuil non atteint, sujet clos jusqu'au backfill. Indépendamment : le worker sur `mcm8` reste en v0.8.0, lot 2 toujours pas vérifié en production (`docker compose up -d loreai` à faire, cf. entrée précédente). |
...
| **Bloqué par** | Rien. Le Pi (`mcm8`) est en ligne ; `loreai-mcp` tourne en v0.12.0, `loreai` (worker) reste en v0.8.0 — voir « Prochain geste ». |
