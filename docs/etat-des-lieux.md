...
**Dernière mise à jour :** 2026-08-29 · **Version publiée :** 0.18.0
...
| | |
|---|---|
| **Lot en cours** | **Aucun — lot 7 ([#48](https://github.com/slucky31/LoreAI/issues/48) + [#78](https://github.com/slucky31/LoreAI/issues/78)) livré.** `MinifluxIngester`/`IFeedIngester` consomme l'API REST d'une instance **Miniflux auto-hébergée** (décision revue en session : Miniflux remplace l'ingestion RSS directe initialement prévue, voir roadmap section « Miniflux auto-hébergé ») — curseur sur l'id d'entrée, pas de backfill au premier démarrage, `FeedIngestionJob` classe chaque entrée sans jamais écrire dans Miniflux ni Raindrop. Déploiement Miniflux ajouté au `docker-compose.yml` + guide (`docs/deploiement-raspberry-pi.md` section 12). PR [#87](https://github.com/slucky31/LoreAI/pull/87) (v0.17.0). En parallèle, O6 (#78) : `WeeklyInsightsJob` envoie désormais un digest Discord natif (deux messages embeds, actionnable/hygiène, 10 entrées + total par section) via `IWeeklyDigestNotifier` au lieu d'une pièce jointe `.md` — `MonthlyReviewJob` inchangé. PR [#88](https://github.com/slucky31/LoreAI/pull/88) (v0.18.0). 379/379 tests verts sur `main`. **Désactivé par défaut** (`Worker__FeedIngestionEnabled=false`) : Miniflux pas encore déployé sur `mcm8`, tout validé via WireMock/NSubstitute. |
| **Prochain geste** | Deux chantiers manuels indépendants, hors CI : (1) **Déployer Miniflux** sur `mcm8` (base + rôle dédiés sur l'instance mutualisée, compte admin, ajout des flux RSS, jeton API, seed `PollingStates SourceType='Feed'`, cf. guide section 12), puis `Worker__FeedIngestionEnabled=true`. (2) **Activer le connecteur Gmail** (lot 8, toujours désactivé en prod faute de projet Google Cloud/client OAuth créé — voir README section dédiée). Une fois l'un ou l'autre en production : **Lot 9 (veille automatique, [#50](https://github.com/slucky31/LoreAI/issues/50)) est débloqué**, construit sur le lot 7. `MonthlyReviewJob` (lot 5) et `WeeklyInsightsJob` (lot 2/7) restent à re-vérifier au prochain passage naturel une fois Miniflux actif. |
| **Dernière décision** | **Lot 7 : Miniflux plutôt qu'ingestion RSS directe, 2026-08-29.** Le roadmap prévoyait un `FeedIngester` parsant du XML (`System.ServiceModel.Syndication`). Reconsidéré au moment d'attaquer le lot : Miniflux auto-hébergé couvre à la fois l'ingestion (via son API REST) et l'interface de lecture humaine (remplace Feedly) en un seul déploiement, cohérent avec l'ADR 0009 qui l'anticipait déjà comme futur locataire de l'instance PostgreSQL mutualisée. Détail dans `docs/roadmap.md`, section « Miniflux auto-hébergé ». |
| **Bloqué par** | Rien. Le Pi (`mcm8`) est en ligne. |

## Ce qui tourne aujourd'hui

**Sur `main`, pas encore sur `mcm8` (voir tableau ci-dessus) :** le worker classe la collection « Non trié » toutes les 15 min (Claude Haiku, tool-use forcé), applique tags et déplacements dans Raindrop, alerte sur Discord les articles `ATester` / `Haute`, et envoie un compte-rendu Discord à la fin de chaque cycle ayant traité au moins un article. Plus d'email : le canal a été retiré (D3, ADR 0013). Persistance PostgreSQL (instance mutualisée sur `mcm8`, EF Core), modèle `Item` générique multi-sources, journal `CycleRuns` et healthcheck Docker (lot 0). `LibraryIndexingJob` (hebdomadaire) indexe en lecture seule toute la bibliothèque Raindrop déjà triée par l'utilisateur (lot 1) ; `WeeklyInsightsJob` (hebdomadaire, juste après) lit cet index et envoie un digest Discord natif (embeds, doublons, tags à nettoyer, collections déséquilibrées, tendances, coût LLM) — zéro appel LLM, zéro écriture (lot 2, revu au lot 7/O6). Connecteurs Gmail (lot 8) et RSS/Miniflux (lot 7) écrits et testés, tous deux désactivés par défaut faute d'activation en production (voir « Prochain geste »). Déploiement par image `ghcr.io` multi-arch, le Pi ne fait que `pull`.

**Réellement en production sur `mcm8` (v0.8.0, non revérifié cette session) :** tout ce qui précède moins le compte-rendu de cycle, moins `WeeklyInsightsJob`, et avec le digest email encore actif à la place du retrait D3.

Le reste de la roadmap (MCP, veille automatique, déduplication inter-sources...) n'est pas encore implémenté.

## Décisions actées, non encore appliquées

| | Décision | Où c'est écrit |
|---|---|---|
| D1 | Hub multi-sources (Raindrop + Gmail + RSS) | [roadmap](roadmap.md), [ADR 0012](adr/0012-modele-item-generique-multi-sources.md) ✅ — modèle `Item`/`ISourceIngester` et les trois connecteurs écrits ; Gmail et RSS/Miniflux restent à activer en production (voir tableau ci-dessus) |
