...
|---|---|---|---|---|
| O7 | **Sauvegarde réelle** — pgBackRest multi-dépôts (SSD + USB + Google Drive), rétention 30j, scaffoldé ; restauration et `.env` chiffré restent à faire | 3 | 1 | F9 |
...
- **O7 — Sauvegarde** ([#37](https://github.com/slucky31/LoreAI/issues/37)) : pgBackRest était déjà en place sur `pg_main` mais jamais planifié (un seul backup complet du 22 août, jamais renouvelé). Élargi au cluster entier (toutes les bases de l'instance mutualisée, pas juste `loreai`) — deux dépôts chiffrés AES-256 (SSD + disque USB), rétention 30j, miroir hors site vers Google Drive (`rclone`). Config committée dans [`mycomicsmanager-config`](https://github.com/slucky31/mycomicsmanager-config), pas dans LoreAI (l'instance ne lui appartient pas, ADR 0009). Reste à déployer sur `mcm8` et à exécuter la restauration **testée une fois** (une sauvegarde jamais restaurée n'est pas une sauvegarde). La copie chiffrée du `.env` de `mcm8` — un actif distinct, qui se perd différemment — n'est pas commencée. Voir F9.
