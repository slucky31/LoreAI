...
|---|---|---|---|---|
| O7 | **Sauvegarde réelle** — pgBackRest (repo chiffré local + miroirs rsync/rclone vers USB et Google Drive), rétention 30j, scaffoldé ; restauration et `.env` chiffré restent à faire | 3 | 1 | F9 |
...
- **O7 — Sauvegarde** ([#37](https://github.com/slucky31/LoreAI/issues/37)) : pgBackRest était déjà en place sur `pg_main` mais jamais planifié (un seul backup complet du 22 août, jamais renouvelé). Élargi au cluster entier (toutes les bases de l'instance mutualisée, pas juste `loreai`) — un dépôt chiffré AES-256 (local, la source de vérité et sa rétention 30j) plus des miroirs `rsync`/`rclone` vers un disque USB et Google Drive, sans politique de rétention séparée. Config committée dans [`mycomicsmanager-config`](https://github.com/slucky31/mycomicsmanager-config), pas dans LoreAI (l'instance ne lui appartient pas, ADR 0009). Reste à déployer sur `mcm8` et à exécuter la restauration **testée une fois** (une sauvegarde jamais restaurée n'est pas une sauvegarde). La copie chiffrée du `.env` de `mcm8` — un actif distinct, qui se perd différemment — n'est pas commencée. Voir F9.
...
| Issue | Lot | Note |
|---|---|---|
| [#37](https://github.com/slucky31/LoreAI/issues/37) Sauvegarde | **11** | Base : pgBackRest multi-dépôts scaffoldé dans `mycomicsmanager-config`, déploiement + restauration à tester sur `mcm8`. `.env` chiffré hors machine : toujours à faire |
