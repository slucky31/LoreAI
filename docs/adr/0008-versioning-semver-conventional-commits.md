...
- **Validation du titre de PR** : l'app GitHub [Semantic PRs](https://github.com/apps/semantic-prs) (déjà utilisée avec succès sur un autre projet de l'utilisateur — l'app "Semantic Pull Requests" de `probot`, essayée en premier, s'est révélée injoignable : aucun status check posté malgré un accès repo confirmé, probablement un service tiers à l'arrêt), configurée via `.github/semantic.yml` (`titleOnly: true`, types restreints à ceux que Versionize sait interpréter). Une règle de branch protection sur `main` rend son check obligatoire avant de pouvoir merger une PR.
...
- **`amannn/action-semantic-pull-request`** : action GitHub qui aurait fait le même travail en s'exécutant dans les propres workflows du repo (donc sans dépendre d'un service tiers) — écartée seulement parce que Semantic PRs (Ezard), déjà éprouvée sur un autre projet de l'utilisateur, remplit le même rôle sans job CI supplémentaire à maintenir.
- **App "Semantic Pull Requests" de `probot`** : essayée en premier car déjà listée comme installée sur le compte avec accès au repo confirmé, mais ne poste jamais de status check (testé sur deux PR distinctes, y compris après un nouvel événement `synchronize`) — service backend probablement hors ligne.
