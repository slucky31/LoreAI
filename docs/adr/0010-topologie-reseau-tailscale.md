...
- **Bénéfice collatéral immédiat** : l'instance PostgreSQL mutualisée devient elle aussi joignable depuis le poste de travail. Cela ouvre l'option d'une base `loreai_dev` sur le Pi pour le développement, sans rien installer localement — utile puisque Docker n'est pas disponible sur le Shadow.
