...
|---|---|---|---|---|
| **F-16** ✅ | Correction | `SqliteConnectionFactory.cs:51-53` | `var command = connection.CreateCommand();` n'était jamais disposé. | Corrigé (`f-16`) : `await using var command`, cohérent avec la connexion déjà en `await using`. |
| **F-17** ✅ | Correction | `RaindropClient.cs:75-78` · `UnsortedClassificationJob.cs:76-78` | `/collections` et `/collections/childrens` étaient concaténés sans déduplication par `Id`, et deux collections peuvent partager le même titre — `FirstOrDefault(c => c.Title == …)` en choisissait une arbitrairement, et l'`enum` du schéma d'outil contenait des doublons. | Corrigé (`f-17`) : déduplication par `Id` côté client, titres dédupliqués dans l'enum et le prompt, et `ResolveTargetCollection` refuse de déplacer sur un titre ambigu (tags appliqués, article laissé dans « Non trié », `LogWarning`). Ranger au hasard serait pire que ne pas ranger. Couvert par un test. |
| **F-18** ✅ | Architecture | `Program.cs:21-29` · `appsettings.json:2-8` | Deux systèmes de niveaux de log cohabitaient : les filtres MEL (`Logging:LogLevel`) et le minimum Serilog (`Serilog:MinimumLevel`, section absente ⇒ Information). Passer `Logging:LogLevel:Default=Debug` restait sans effet, Serilog filtrant en aval. | Corrigé (`f-18`) : `Logging` supprimée des deux `appsettings`, remplacée par une section `Serilog` (`MinimumLevel` + `Override`) qui porte aussi `FilePath`. Vérifié en exécution — `Serilog__MinimumLevel__Default=Warning` seul laisse passer les logs de `Microsoft.Hosting.Lifetime` (override à `Information`), et les fait bien taire quand on lève aussi cet override. |
...
| **F-23** ✅ | Sécurité | `.github/workflows/*.yml` | Les actions tierces étaient référencées par tag mobile (`@v4`, `@v6`, `marocchino/sticky-pull-request-comment@v2`) et non par SHA. Or `cd.yml` dispose de `contents: write`, `packages: write` et d'un PAT admin (`RELEASE_TOKEN`) : une action compromise s'exécuterait avec ces droits. | Corrigé (`f-23`) : les 8 actions épinglées par SHA de commit, avec la version en commentaire. SHA résolus via l'API GitHub, **en restant sur la majeure en place** — épingler n'est pas monter de version, et `actions/checkout` en est à v7. Preset `helpers:pinGitHubActionDigests` ajouté pour que Renovate maintienne les digests. La restriction de `RELEASE_TOKEN` reste ouverte : elle dépend de la configuration du compte, pas du dépôt. |
| **F-24** ✅ | Design | `ClassificationResponseParser.cs:13-34` | L'exception était le flux de contrôle nominal : le parser levait pour signaler une sortie LLM invalide, et l'unique appelant rattrapait via un `catch (Exception)` très large qui masquait aussi bien un JSON malformé qu'un bug de programmation. | Corrigé (`f-24`) : `TryParse` et `TryExtractToolInput` renvoient un booléen + un message. Le `catch` se réduit à `IsTransportFailure` — HTTP, JSON, timeout, et l'annulation non demandée — ; une `NullReferenceException` de notre code remonte désormais au job, qui la journalise en erreur avec sa pile au lieu de la déguiser en « classification échouée ». L'annulation demandée continue de remonter intacte (F-06). |
| **F-25** ✅ | Correction | `RaindropClient` · `ArticleRepository` · `EmailNotifier` | Aucun `ILogger` : rien n'était tracé sur la pagination, le nombre de lignes affectées, ni l'envoi SMTP. Le diagnostic reposait entièrement sur les logs du job appelant. | Corrigé (`f-25`, `RaindropClient` dès `f-12`) : `ILogger<T>` injecté dans les trois. Trace utile plutôt que verbeuse — plafond de pagination atteint, envoi SMTP avec hôte/port/mode de sécurité, et surtout un `LogWarning` quand `RecordWriteBackAsync` ne touche aucune ligne, cas jusqu'ici parfaitement silencieux. |
