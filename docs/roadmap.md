...
| L5 | **File taguée « cette-semaine »**, alimentée par L1 | 2 | 2 | ✅ Livré (validation explicite donnée en session, 2026-08-29) — via un **tag** Raindrop, pas une vraie collection : un item n'appartient qu'à une seule collection à la fois, y déplacer l'article le retirerait de sa collection thématique déjà classée. Fait de Raindrop l'interface de lecture sans rien casser du rangement existant. Implique une écriture **hors « Non trié »** (voir Risques) — `Worker__ReadingQueueTaggingEnabled=false` par défaut |
...
Sujets provisionnés par commande → flux de recherche (RSS) → filtrage LLM contre le corpus existant pour ne remonter que ce qui est nouveau et pertinent → création directe dans Raindrop.

Construit sur le lot 7, comme prévu, avec la contrainte RSS-d'abord respectée (jamais de recherche web facturée à l'appel — voir l'arbitrage ci-dessous). **Design revu en cours de lot (2026-08-30)** : le design initial (alerte Discord détaillée par article, sujets en config statique, jamais d'écriture Raindrop) a été remplacé après retour utilisateur par une écriture directe — chaque sujet obtient sa **propre collection Raindrop et sa propre catégorie Miniflux**, provisionnées en une commande (`--add-watch-topic --name=... --description=...`, même patron que `--health-check`), persistées dans une table `WatchTopics` (curseur d'ingestion par sujet, plus par `SourceType` partagée). `MinifluxWatchIngester` (`IMinifluxCategoryReader`, lecteur pur sans état) lit `GET /v1/categories/{id}/entries` pour la catégorie du sujet ; `ITopicWatchFilter`/`AnthropicTopicWatchFilter` (tool-use forcé, même patron que `AnthropicClassifier`) juge un seul sujet à la fois (déjà connu via l'ingestion par catégorie) et propose des tags réutilisant le vocabulaire existant. La « nouveauté » contre le corpus reste basée sur la recherche plein texte déjà en place (`ICorpusQueryRepository.SearchAsync`, Q2/lot 5). Une entrée jugée pertinente **et** nouvelle est créée directement comme raindrop dans la collection du sujet (`IRaindropClient.CreateRaindropAsync`, tag `veille` + tags proposés) — **seul cas du projet où une source non-Raindrop provoque une création dans Raindrop**, jamais une modification d'un item existant (ADR 0012 intact sur ce point précis). La notification Discord détaillée par article est remplacée par un résumé groupé par exécution (`IWatchDigestNotifier`, même règle « pas d'import, pas de notification » qu'O1) ; un log d'audit minimal (`WatchEvaluationLogs`) reste la seule trace pour S6. Désactivé par défaut (`Worker__TopicWatchEnabled=false`). L'idée de réinjecter aussi les newsletters Gmail (lot 8) dans des collections Raindrop, soulevée en miroir pendant cette session, est trackée séparément dans l'issue [#94](https://github.com/slucky31/LoreAI/issues/94) — hors scope ici.
...
| **Écriture hors « Non trié »** (N1 avec tag, L5) | Invariant historique du projet. Chaque écriture hors périmètre exige une décision explicite et son propre flag, jamais un effet de bord — première application : L5 (`ReadingQueueTaggingJob`, `Worker__ReadingQueueTaggingEnabled=false` par défaut, tag plutôt que collection pour ne jamais toucher au rangement thématique existant) |
